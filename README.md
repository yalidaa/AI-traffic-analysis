...
MineShark-Tor 是一个面向毕业小论文和简历项目沉淀的 Tor 加密流量行为识别原型系统。当前研究主线是：
...
当前阶段优先做 **Tor WF 权威数据集专项**，暂不急着堆模型。

原因很简单：毕业小论文不能建立在“不知道数据集是否权威、是否最新、是否常被顶会使用”的基础上。第一阶段先筛选和验证数据集，再进入 DTB-Fusion 与完整实验。
