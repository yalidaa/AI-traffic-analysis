...
install -d -o root -g root -m 0755 /var/empty/mineshark/zeek /var/empty/mineshark/suricata
...
for _attempt in $(seq 1 60); do
  if curl --fail --silent --insecure --user "admin:${admin_password}" https://127.0.0.1:9200/ >/dev/null; then
    break
  fi
  sleep 2
done
curl --fail --silent --insecure --user "admin:${admin_password}" https://127.0.0.1:9200/ >/dev/null
